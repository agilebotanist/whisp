...
### 2.2 Model: `large-v3` by default (revised three times from an original `large-v3-turbo` default)

This project's default model moved three times during real use, each time for a concrete, observed reason rather than a hypothetical one — worth recording the full sequence, since the reasoning at each step is what would need to be re-evaluated if this is ever revisited, and because the final answer ended up being counterintuitive relative to the first two steps.

**`large-v3-turbo` (original default).** OpenAI's pruned-decoder variant of `large-v3`, several times faster than the full model for a generally imperceptible accuracy cost — correct as far as it went, but the wrong comparison. Dogfooding surfaced the real constraint: on a 16GB Mac mini, a ~2-hour real-world file took over two hours and never finished during a full working session, because the machine had well under 100MB of free RAM at the time (a multi-process browser and an editor were themselves holding several GB) and swap climbed to ~8GB. Apple Silicon's unified memory means GPU compute directly competes with whatever else is resident; "fast" only holds when there's memory headroom for it.

**`small` (first revision).** Needs meaningfully less memory and less compute per frame than turbo, so it stayed fast without that headroom — directly fixing the scenario above. It worked: the same ~2-hour file completed in about 3 minutes (roughly 36x realtime).

**`medium` (second revision).** With `small`'s speed proven, a direct side-by-side comparison was run against `large-v3-turbo` on the same file to judge whether turbo's accuracy was worth its cost. It wasn't just slow — partway through, frame progress stopped advancing entirely for 2+ minutes while `sample`-based process inspection showed genuinely heavy, continuous compute activity the whole time (not a hang or deadlock — real matmul/eval calls, just making no forward progress on the frame counter). One specific decode window got stuck, reproducibly, under the same memory conditions that made turbo slow in the first place; `medium` was tried as a smaller step up from `small` and climbed smoothly with no such stall, becoming the default on that basis.

**`large-v3` (current default).** Tried next, expecting it to be worse than turbo (it's the larger model turbo is distilled from) — and its first ~90 seconds looked exactly that bad: real progress, but at roughly 1x realtime, which would have meant ~2 hours for the same file. It was let keep running anyway, and the assumption turned out wrong: once past that initial window, it accelerated substantially, finishing the full ~2-hour file in about 12.5 minutes total (~9x realtime) — slower than `medium`, but not the multi-hour outcome the early reading implied, and with no stall. The lesson worth keeping: an early-elapsed extrapolation for *any* model on this machine can be seriously misleading in either direction while the system is still settling memory around a newly-loaded model — a real reason (on top of REQUIREMENTS' NFR-1) to prefer the actual frame-based percentage (§2.10) over a naive "elapsed so far / percent so far" ETA, and to let a model run past its first minute or two before judging its speed. `large-v3` is the default because, once past that initial window, its accuracy is worth its cost on this machine; `medium`, `small`, `base`, and `tiny` remain one flag away for faster turnaround when a given file doesn't need the extra accuracy.
